...
public class SHA0 {
    private static final int SHA0_BLOCK_SIZE = 64; // Block size in bytes
    private static final int SHA0_HASH_SIZE = 20;  // Hash size in bytes
    ...
    public SHA0() {
        intermediateHash = new int[SHA0_HASH_SIZE / 4];
        messageBlock = new byte[SHA0_BLOCK_SIZE];
        reset();
    }
    ...
    public void update(byte[] data) {
        ...
        for (byte b : data) {
            ...
            if (messageBlockIndex == SHA0_BLOCK_SIZE) {
                processMessageBlock();
                messageBlockIndex = 0;
            }
        }
    ...
    public byte[] digest() {
        ...
        byte[] hash = new byte[SHA0_HASH_SIZE];
        ByteBuffer buffer = ByteBuffer.allocate(SHA0_HASH_SIZE).order(ByteOrder.BIG_ENDIAN);
    ...
    private void padMessage() {
        ...
        if (messageBlockIndex > 55) {
            ...
            while (messageBlockIndex < SHA0_BLOCK_SIZE) {
    ...
    private void processMessageBlock() {
        ...
        // SHA-0: Simply XOR the previous 16 words without any rotation
        for (int t = 16; t < 80; t++) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Message digest of SHA0 = " + hexString.toString().trim());
    }
}
